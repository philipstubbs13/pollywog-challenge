...
export const Artwork = (props) => {
  ...
  useEffect(() => {
    props.artFavoritesDB().then(db => db.transaction('favorite_art')
      ...
      if (obj.length) {
        const savedItem = obj.filter(item => item._source.id === props.match.params.id);
  ...
  const onSaveToFavorites = () => {
    const artwork = props.artItems.filter(item => item._source.id === props.match.params.id);
    props.artFavoritesDB().then(db => db.transaction('favorite_art', 'readwrite')
  ...
  return (
    <>
      {props.artItems.filter(item => item._source.id === props.match.params.id).map(item => (
        <div className={classes.artworkContainer} key={item._source.id}>
          <div className={classes.art}>
            <img
              src={`https://1.api.artsmia.org/${props.match.params.id}.jpg`}
